...
class SimpleAutomation:
    ...
    def _handle_click_action(self, action_config: Dict[str, Any], target_element: Optional[BoundingBox]) -> bool:
        ...
        click_type = action_config.get("clickType", "win32").lower()  # default to win32
        ...
        action = {
            ...
            "y": y,
            "button": button,
            "clickType": click_type,  # send backend method
        ...
        try:
            ...
            logger.error(f"Failed to click on {element_info} using {click_type}: {str(e)}")
            return False
    ...
    def _handle_keyboard_action(self, action_config: Dict[str, Any]) -> bool:
        ...
        if action_type == "hotkey":
            ...
            method_key = action_config.get("methodType")
            if method_key is None:
                # fallback if not provided
                method_key = {"default": "pyautogui"}

            try:
                response = self.network.send_action({"type": "key", "key": mapped_key, "methodType": method_key})
                logger.info(f"Pressed key: {mapped_key} {key.lower()} with methodType: {method_key}")
                return True
            except Exception as e:
    ...
    def _find_matching_element(self, target_def, bounding_boxes):
        ...
        for bbox in bounding_boxes:
            ...
            if type_match and text_match:
                ...
                logger.debug(f"[OK] Match found: {bbox.element_type} '{element_text}' at ({bbox.x}, {bbox.y})")
                return bbox

        logger.debug("[FAIL] No matching element found")
        return None
