...
const RPX_BACKEND_URL = '/api'; // Proxied to localhost:5000
...
/**
 * Parse a screenshot with OmniParser (via Raptor X backend)
...
export async function parseScreenshot(
  ...
  const response = await fetch(`${RPX_BACKEND_URL}/omniparser/analyze`, {
    method: 'POST',
    body: formData,
  });
 ...
 * This routes through Raptor X backend to start an automation run
 */
export async function runWorkflow(
  ...
  const response = await fetch(`${RPX_BACKEND_URL}/runs`, {
    method: 'POST',
...
export async function listWorkflows(): Promise<WorkflowSummary[]> {
  const response = await fetch(`${RPX_BACKEND_URL}/workflows`);
...
export async function getWorkflowYaml(gameName: string): Promise<string> {
  const response = await fetch(
    `${RPX_BACKEND_URL}/games/${encodeURIComponent(gameName)}/yaml`
  );
...
export async function createWorkflow(name: string, yaml: string): Promise<{
  ...
  const response = await fetch(`${RPX_BACKEND_URL}/games`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
export async function deleteWorkflow(gameName: string): Promise<{
  ...
  const response = await fetch(
    `${RPX_BACKEND_URL}/games/${encodeURIComponent(gameName)}`,
    { method: 'DELETE' }
  );
...
export async function validateWorkflowYaml(gameName: string, yaml: string): Promise<{
  ...
  const response = await fetch(
    `${RPX_BACKEND_URL}/games/${encodeURIComponent(gameName)}/validate`,
    {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ yaml }),
