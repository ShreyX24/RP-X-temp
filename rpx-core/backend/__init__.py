...
def _find_admin_dir() -> Path | None:
    ...
    if env_admin := os.environ.get("RPX_ADMIN_DIR"):
...
def _launch_frontend():
    ...
    if not admin_dir:
        print("Warning: Admin directory not found. Set RPX_ADMIN_DIR environment variable.")
        return
    ...
    if sys.platform == "win32":
        subprocess.Popen(
            f'wt -w 0 nt --title "rpx [frontend]" --suppressApplicationTitle -d "{admin_dir}" cmd /k "npm run dev -- --host"',
            shell=True
        )
    else:
        terminals = [
            ["gnome-terminal", "--tab", "--title=rpx (frontend)", "--", "bash", "-c", "npm run dev -- --host; exec bash"],
            ["xterm", "-T", "rpx (frontend)", "-e", "npm run dev -- --host"],
        ]
...
def main():
    """Main entry point for RPX backend"""
    parser = argparse.ArgumentParser(
        description="Raptor X - Game Automation Platform",
