...
class SUTDiscoveryService:
    ...
    def _scan_ip(self, ip: str) -> Optional[SUTDevice]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Verify this is an RPX SUT
                if self._is_rpx_sut(data):
                    return self._process_sut_response(ip, data)
                else:
                    logger.debug(f"Non-RPX service found at {ip}:{self.config.sut_port}")
    ...
    def _is_rpx_sut(self, response_data: Dict[str, Any]) -> bool:
        """Check if response indicates an RPX SUT (supports both new and legacy signatures)"""
        # Check for new RPX identifier
        ...
        # Fallback: check for RPX-specific capabilities
        ...
        # If it has most RPX capabilities, it's probably an RPX SUT
        matching_caps = sum(1 for cap in rpx_capabilities if cap in capabilities)
